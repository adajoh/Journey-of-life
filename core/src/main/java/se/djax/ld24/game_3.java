...
/* Decompiler 176ms, total 372ms, lines 246 */
...
public class game_3 implements Screen {
	...
	float startDelay;
	hud hud;
	game_3.TileMap tileMap;
	game_3.Camera camera;
	Player player;
	Random random;
	ArrayList<game_3.Npc> npcs;
	boolean isOnNpc;
	...
	public game_3() {
		...
		this.hud = new hud(3);
		this.tileMap = new game_3.TileMap();
		this.player = new Player(3);
		this.camera = new game_3.Camera((game_3.Camera) null);
		this.camera.entityToFollow = this.player;
		this.player.x = 1.0F;
		this.player.y = 0.0F;
		this.npcs = new ArrayList<Npc>();
		this.npcs.add(new game_3.Npc(0));
		this.npcs.add(new game_3.Npc(0));
		this.npcs.add(new game_3.Npc(0));
		this.npcs.add(new game_3.Npc(1));
	}
	...
	public void render(SpriteBatch batch) {
		if (this.state > 1) {
			...
			Iterator<Npc> var3 = this.npcs.iterator();

			while (var3.hasNext()) {
				game_3.Npc npc = var3.next();
				this.renderEntity(batch, npc);
			}

			this.renderEntity(batch, this.player);
	...
	public void update(float rawDeltaTime) {
		...
		if (this.state == 2) {
			game_3.Camera var10000 = this.camera;
			var10000.zoom -= 0.005F;
		...
		public TileMap() {
			...
			for (int x = 0; x < this.tilemap2.length; ++x) {
				for (int y = 0; y < this.tilemap2[x].length; ++y) {
					this.tilemap2[x][y] = game_3.this.new Tile();
